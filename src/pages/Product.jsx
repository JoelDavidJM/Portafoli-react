...
const Product = () => {
  ...
  return (
    ...
    <h1 className='contact__mensage'>{t("MESSAGE")}</h1>
    <button onClick={handelExit} className={`close ${click && 'open btn'}`}>{t("ACCEPT")}</button>
    </section>
    </div>
      <h1 className='contact__container__title'>{t("CONTACT1")}</h1>
       <section className='contact__from'>
        <form ref={form} onSubmit={sendEmail} className='field'>
      <label className='form__title__name'>{t("NAME")}</label>
      ...
      <label className='form__title__message'>{t("MESSAGE1")}</label>
      <textarea className='from__message' name="message" />
      <input onClick={handelClick} type="submit" value={t("SEND")}/>
    </form>
